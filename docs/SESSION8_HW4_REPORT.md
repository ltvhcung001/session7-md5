...
jobs:
  pull_and_verify:
    ...
    steps:
      ...
      - name: Pull and Run Payment Service Container
        run: |
          # 1. Chuyển đổi tên owner sang chữ thường để bảo đảm hợp lệ chuẩn GHCR
          OWNER=$(echo "${{ github.repository_owner }}" | tr '[:upper:]' '[:lower:]')
